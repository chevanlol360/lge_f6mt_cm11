...
/* Workaround for improvement of rock-bottom current
  * 2013-04-04, junsin.park@lge.com
  */
	...
	/* LGE_UPDATE_S [dongwon.choi@lge.com] 2013-04-15
	 * gpio 140, 141 are set output although these are not connected(nc) pins
	 * workaround for improvement of rock-bottom
	 * until find out where these settings are done
	 */
	...
	gpio_tlmm_config(GPIO_CFG(140, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		 ...
		 * print gpio dump when ap power collapsed */
		gpio_debug_print();
		/* LGE_UPDATE_E */
#endif /* CONFIG_LGE_PM */
