...
#include <linux/input/touch_synaptics_l9ii.h>

#include "SynaImage-l9ii.h"
	...
	*total_num = 0;
	...
	if (likely(ts->ts_data.interrupt_status_reg & ts->interrupt_mask.abs)) {
		...
		while(touch_finger_bit_mask) {
			...
			if (unlikely(touch_debug_mask & DEBUG_GET_DATA)) {
				TOUCH_INFO_MSG("Finger_index : %d\n",finger_index);
			}
			
			if (unlikely(index >= MAX_NUM_OF_FINGERS)) {
				TOUCH_ERR_MSG("index(%d) exceed MAX_NUM_OF_FINGERS(%d)\n", index, MAX_NUM_OF_FINGERS );
				goto err_synaptics_getdata;
			}
...
// LGE_CHANGE_S [hoseong.han@lge.com] 2013-04-18 L9II WBT INTERNAL_TEST WBT_12 TD 477824
	u8 buf = 0;
