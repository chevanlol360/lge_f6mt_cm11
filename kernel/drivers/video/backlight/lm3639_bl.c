...
//LGE_CHANGE_S [koh.euije@lge.com]2013.05.20 move from lower position
/* Flash control for L9II */
/* 2013-04-25 jinsang.yun@lge.com */
...
//LGE_CHANGE_E [koh.euije@lge.com]2013.05.20 move from lower position

// LGE_CHANGE_S [younglae.kim@lge.com] 2013-11-26, add for debug log
unsigned char debug_mask = 0;
// LGE_CHANGE_E [younglae.kim@lge.com] 2013-11-26
...
char level2bright_per(int level)
{
	...
	if(unlikely(debug_mask))
		printk("[Backlight] %s: bright_per=0x%x, level=0x%x\n", __func__, bright_per, level);
...
unsigned int get_map_from_level(int level)
{
	...
	if(unlikely(debug_mask))
		printk("[Backlight] %s: level=0x%x, map=0x%x\n", __func__, level, map);
...
unsigned char cal_reg_from_level(int level)
{
	...
	if(unlikely(debug_mask)) {
		printk("[Backlight] %s: level=0x%x, brt_per:%d%%, max_reg_value=0x%x, min_reg_value=0x%x, "
				"max_level=0x%x, min_level=0x%x, calc_reg_value=0x%x\n",
			__func__, level, (int)level2bright_per(level), max_reg_value, min_reg_value,
			max_level, min_level, calc_reg_value);
	}
...
{
	if(unlikely(debug_mask))
		printk("[Backlight] %s: reg_addr=0x%x, reg_value=0x%x\n",
				__func__, reg_data->addr, reg_data->value);
...
static void lm3639_set_main_current_level(struct i2c_client *client, int level)
{
	...
	//LGE_CHANGE_S [koh.euije@lge.com]2013.05.20 TD #30209
	unsigned char bl_ctrl;
	//LGE_CHANGE_E [koh.euije@lge.com]2013.05.20 TD #30209

	if(unlikely(debug_mask))
		printk("[Backlight] %s: level=%d\n", __func__, level);
...
//LGE_CHANGE_S [koh.euije@lge.com] 2013.04.15 because of not intentional changing brightness...
	if (level == -1)
		level = cur_main_lcd_level; //dev->default_brightness;
//LGE_CHANGE_E [koh.euije@lge.com] 2013.04.15 because of not intentional changing brightness...
	...
	if (level != 0) {
		...
		if (!(lm3639_onoff_state & LM3639_BL_ON)) {
			//LGE_CHANGE_S [koh.euije@lge.com] 2013.04.12 OVP 24V -> 32V
			lm3639_write_reg(main_lm3639_dev->client, REG_BL_CONF_2, 0x54);
			//LGE_CHANGE_E [koh.euije@lge.com] 2013.04.12 OVP 24V -> 32V
			//LGE_CHANGE_S [koh.euije@lge.com] 2013.04.16 Ramp-up/down step time. Need to optimize
			lm3639_write_reg(main_lm3639_dev->client, REG_BL_CONF_3, 0x01);
			//LGE_CHANGE_E [koh.euije@lge.com] 2013.04.16 Ramp-up/down step time. Need to optimize
...
void lm3639_enable_torch_mode(enum led_status state)
{
	...
	//jongho3.lee@lge.com[2013-06-05] : Since default is FLASH(0x04) mode after lm3639 power on,
...
static void lm3639_config_gpio_on(void)
{
	...
	if (gpio_get_value(gpio) != 1) {
		...
		//LGE_CHANGE_S [koh.euije@lge.com] 2013.04.12 add delay
		mdelay(10);
		//LGE_CHANGE_E [koh.euije@lge.com] 2013.04.12 add delay
		printk("[Backlight] %s[%d]: ON!\n", __func__, gpio);
	}

	if(unlikely(debug_mask))
		printk("[Backlight] %s --<< gpio=%d\n", __func__, gpio_get_value(gpio));
}
...
int lm3639_flash_set_led_state(int led_state)
{
	...
	if(unlikely(debug_mask))
		printk("[Backlight] %s: led_state = %d\n", __func__, led_state);
	...
	switch (led_state) {
		...
		case MSM_CAMERA_LED_HIGH:
			...
			if(batt_temp > -100) {
				pr_err("[Backlight] %s: Working on LED_HIGH Flash mode (Battery temperature = %d)\n",
						__func__, batt_temp);
				lm3639_enable_flash_mode(LM3639_LED_HIGH);
			} else {
				pr_err("[Backlight] %s: Working on LED_LOW Flash mode (Battery temperature = %d)\n",
						__func__, batt_temp);
...
{
	if(unlikely(debug_mask))
		printk("[Backlight] %s: led_cdev->brightness[%d]\n", __func__, value);
...
// LGE_CHANGE_S [younglae.kim@lge.com] 2013-11-26, add for debug log
static ssize_t lm3639_debug_mask_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", debug_mask);
}

static ssize_t lm3639_debug_mask_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int value;
	sscanf(buf, "%d", &value);

	debug_mask = (unsigned char)value;

	return count;
}

static DEVICE_ATTR(debug_mask, S_IRUGO | S_IWUSR, lm3639_debug_mask_show, lm3639_debug_mask_store);
// LGE_CHANGE_E [younglae.kim@lge.com] 2013-11-26
...
static int lm3639_probe(struct i2c_client *i2c_dev, const struct i2c_device_id *id)
{
	...
	pr_info("[Backlight] %s: i2c probe start\n", __func__);
...
// LGE_CHANGE_S [younglae.kim@lge.com] 2013-11-26, add for debug log
	err = device_create_file(&i2c_dev->dev, &dev_attr_debug_mask);
// LGE_CHANGE_E [younglae.kim@lge.com] 2013-11-26
...
// LGE_CHANGE_S [younglae.kim@lge.com] 2013-11-26, add for debug log
	device_remove_file(&i2c_dev->dev, &dev_attr_debug_mask);
// LGE_CHANGE_E [younglae.kim@lge.com] 2013-11-26
