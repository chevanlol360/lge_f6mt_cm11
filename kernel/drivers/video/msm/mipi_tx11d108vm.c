...
static struct dsi_buf tx11d108vm_tx_buf;
static struct dsi_buf tx11d108vm_rx_buf;
 ...
 */
static int mipi_tx11d108vm_lcd_on(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "%s: mipi tx11d108vm lcd on started \n", __func__);
	mipi_dsi_cmds_tx(&tx11d108vm_tx_buf, mipi_tx11d108vm_pdata->power_on_set,
			mipi_tx11d108vm_pdata->power_on_set_size);
...
static int mipi_tx11d108vm_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	mfd = platform_get_drvdata(pdev);
	if (!mfd)
	...
	printk(KERN_INFO "%s: mipi tx11d108vm lcd off started \n", __func__);
	mipi_dsi_cmds_tx(&tx11d108vm_tx_buf,
			mipi_tx11d108vm_pdata->power_off_set,
			mipi_tx11d108vm_pdata->power_off_set_size);
	return 0;
}
...
static void mipi_tx11d108vm_set_backlight_board(struct msm_fb_data_type *mfd)
{
	struct mipi_panel_info *mipi;

	if ((mipi_tx11d108vm_pdata->enable_wled_bl_ctrl)
...
static void mipi_tx11d108vm_set_backlight_board(struct msm_fb_data_type *mfd)
{
	...
	mipi_tx11d108vm_pdata->backlight_level(level, 0, 0);
}
#endif
...
#endif /* CONFIG_LGE_ESD_CHECK */
static int mipi_tx11d108vm_lcd_probe(struct platform_device *pdev)
{
...
#endif /* CONFIG_LGE_ESD_CHECK */
	if (pdev->id == 0) {
		mipi_tx11d108vm_pdata = pdev->dev.platform_data;
		return 0;
	}

	printk(KERN_INFO "%s: mipi tx11d108vm lcd probe start\n", __func__);
	pdata = pdev->dev.platform_data;
	if (!pdata)
		return 0;
#if !(defined(CONFIG_MACH_LGE_F6_TMUS)|| defined(CONFIG_MACH_LGE_F6_VDF) || defined(CONFIG_MACH_LGE_F6_ORG)|| defined(CONFIG_MACH_LGE_F6_OPEN) || defined(CONFIG_MACH_LGE_F6_TMO))
	pdata->panel_info.bl_max = mipi_tx11d108vm_pdata->max_backlight_level;
#endif
...
static struct platform_driver this_driver = {
	.probe  = mipi_tx11d108vm_lcd_probe,
	.driver = {
		.name   = "mipi_tx11d108vm",
	},
};

static struct msm_fb_panel_data tx11d108vm_panel_data = {
	.on		= mipi_tx11d108vm_lcd_on,
	.off		= mipi_tx11d108vm_lcd_off,
	.set_backlight = mipi_tx11d108vm_set_backlight_board,
};

static int ch_used[3];

int mipi_tx11d108vm_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_tx11d108vm", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	tx11d108vm_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &tx11d108vm_panel_data,
		sizeof(tx11d108vm_panel_data));
	if (ret) {
		printk(KERN_ERR
...
static int __init mipi_tx11d108vm_lcd_init(void)
{
#if defined(CONFIG_MACH_LGE_F6_TMUS)|| defined(CONFIG_MACH_LGE_F6_VDF) || defined(CONFIG_MACH_LGE_F6_ORG)|| defined(CONFIG_MACH_LGE_F6_OPEN) || defined(CONFIG_MACH_LGE_F6_TMO)
	...
	mipi_dsi_buf_alloc(&tx11d108vm_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&tx11d108vm_rx_buf, DSI_BUF_SIZE);

	return platform_driver_register(&this_driver);
}

module_init(mipi_tx11d108vm_lcd_init);
