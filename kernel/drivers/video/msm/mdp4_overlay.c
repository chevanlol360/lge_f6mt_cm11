...
#endif
/* [END] force FLIP_MODE for INVERSE PANEL - hoseok.kim@lge.com */
...
void mdp4_overlay_rgb_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	if (panel_rotate_180 && (pipe->pipe_num == OVERLAY_PIPE_RGB1 || pipe->pipe_num == OVERLAY_PIPE_RGB2))
	{
		...
		/* LGE_CHANGE_S [jake.noh@lge.com] 2013-08-20, Change 180 degree rotation routine to fix wrong position display*/
  	    if (pipe->mfd)		
			dst_xy = (((pipe->mfd->panel_info.yres - pipe->dst_y - pipe->dst_h) << 16) | pipe->dst_x);
		else
			pr_err("rgb mfd is not set\n");
		/* LGE_CHANGE_E [jake.noh@lge.com] 2013-08-20*/
	}
#endif
	/* [END] force FLIP_MODE for INVERSE PANEL - hoseok.kim@lge.com */
				 ...
				 u32 src_h, u32 dst_h, u32 src_w, u32 dst_w)
{
	...
	if (rst < pclk) {
	/*
	 * LGE_CHANGE [younglae.kim@lge.com] 2013-12-09,
	 * patch from JB to fix underrun which is occured during camera resume
	 */
