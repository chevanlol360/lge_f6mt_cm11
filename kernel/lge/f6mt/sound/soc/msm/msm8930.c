...
static int msm8930_auxpcm_rate = BTSCO_RATE_8KHZ;  // LGE, used in LGE's own functions
...
static const struct snd_soc_dapm_route common_audio_map[] = {
	...
	/* Microphone path */
	// LGE_AUDIO_BSP , ADC3 needs MIC BIAS1 in L9II
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if(ucontrol->value.integer.value[0]==1){
		...
		// LGE_AUDIO_BSP_S  seungdae.goh@lge.com  2014-03-08 no sound -> IRRC  speaker unmute bug fix
		if( !msm8930_ext_spk_pamp) {
			printk("Do not Unmute Speaker Not AMP ON" );
			return ret;
		}
		// LGE_AUDIO_BSP_E  seungdae.goh@lge.com  2014-03-08 no sound -> IRRC  speaker unmute bug fix
	}
