		...
		*intstatus)
{
	...
	printk("[Knock_On] %s : comres[%d], data[%d]\n", __func__, comres, data);
	*intstatus = data;
...
static int bma2x2_calibration(struct bma2x2_data *bma2x2, int axis)
{
	...
	struct bma2x2acc xyz, pre_xyz;
	enum { X, Y, Z };

	bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type, &pre_xyz);

    for(shaking_cnt=0; shaking_cnt<10; shaking_cnt++) {
        mdelay(20);
        bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type, &xyz);
		printk("%s: (%d, %d), (%d, %d), (%d, %d)\n", __func__, xyz.x, pre_xyz.x, xyz.y, pre_xyz.y, xyz.z, pre_xyz.z);

		if( ((abs(xyz.x - pre_xyz.x) > BMA2X2_SHAKING_DETECT_THRESHOLD)
			|| (abs((xyz.y - pre_xyz.y)) > BMA2X2_SHAKING_DETECT_THRESHOLD)
			|| (abs((xyz.z - pre_xyz.z)) > BMA2X2_SHAKING_DETECT_THRESHOLD)) )
        {
            printk("%s: %d-axis calibration is failed due to BMA2X2_SHAKING\n", __func__, axis+1);
			printk("%s: (%d, %d), (%d, %d), (%d, %d)\n", __func__, xyz.x, pre_xyz.x, xyz.y, pre_xyz.y, xyz.z, pre_xyz.z);
            return -EINVAL;
        }
        else
    ...
    printk("====================== complete shaking check for %d-axis ======================\n", axis+1);

    switch(axis) {
        case X :
            offset = (unsigned char)BMA2X2_OFFSET_TARGET_X;
			printk("%s: START fast calibration for X-axis\n", __func__);
            break;
        case Y :
            offset = (unsigned char)BMA2X2_OFFSET_TARGET_Y;
			printk("%s: START fast calibration for Y-axis\n", __func__);
            break;
        case Z :
            offset = (unsigned char)BMA2X2_OFFSET_TARGET_Z;
    ...
    if(bma2x2_set_offset_target(bma2x2->bma2x2_client, (unsigned char)(axis+1), offset) < 0)
        return -EINVAL;

	if(bma2x2_set_cal_trigger(bma2x2->bma2x2_client, (unsigned char)(axis+1)) < 0)
		return -EINVAL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", atomic_read(&bma2x2->calibration_result));
}
		...
		const char *buf, size_t count)
{
	...
	atomic_set(&bma2x2->calibration_result, 0);
	atomic_set(&cal_result, 0);

	printk("===================================================\n%s: START accel. sensor calibration\n", __func__);

	if(bma2x2_get_bandwidth(bma2x2->bma2x2_client, &bw) < 0)
		return -EINVAL;

	if(bma2x2_set_bandwidth(bma2x2->bma2x2_client,	BMA2X2_BW_15_63HZ) < 0)
		return -EINVAL;
    printk("%s: bandwidth is set from %d to 9\n", __func__, bw);
    mdelay(100);

	for(idx=0; idx<3; idx++) {
		if( bma2x2_calibration(bma2x2, idx) < 0 ) {
	...
	if(atomic_read(&cal_result) == 3) {
		atomic_set(&bma2x2->calibration_result, 1);
		printk("%s: PASS!!!!!\n", __func__);
	} else {
		printk("%s: FAIL!!!!!\n", __func__);
		return -EINVAL;
...
static int bma2x2_read_calibration_data(struct i2c_client *client)
{
	...
	fd_offset_x = sys_open("/sns/bmc_cal/offset_x.dat",O_RDONLY, 0);
	fd_offset_y = sys_open("/sns/bmc_cal/offset_y.dat",O_RDONLY, 0);
	fd_offset_z = sys_open("/sns/bmc_cal/offset_z.dat",O_RDONLY, 0);
		...
		const char *buf, size_t count)
{
	...
	fd_offset_x = sys_open("/sns/bmc_cal/offset_x.dat",O_WRONLY|O_CREAT, 0666);
	fd_offset_y = sys_open("/sns/bmc_cal/offset_y.dat",O_WRONLY|O_CREAT, 0666);
	fd_offset_z = sys_open("/sns/bmc_cal/offset_z.dat",O_WRONLY|O_CREAT, 0666);	
...
struct lge_touch_attribute {
	...
	ssize_t (*show)(struct bma2x2_data *bma2x2, char *buf);
	ssize_t (*store)(struct bma2x2_data *bma2x2, const char *buf, size_t count);
};
#define LGE_TOUCH_ATTR(_name, _mode, _show, _store) \
struct lge_touch_attribute lge_touch_attr_##_name = __ATTR(_name, _mode, _show, _store)

static LGE_TOUCH_ATTR(lpwg_notify, S_IRUGO|S_IWUSR|S_IWGRP,
				...
				const char *buf, size_t count)
{
	...
	if (lge_touch_priv->store)
		ret = lge_touch_priv->store(bma2x2, buf, count);
...
//	bma2x2_set_int1_pad_sel(client, PAD_ORIENT);
	...
	gpio_request(BMA2x2_INT, "bma2x2_irq");
	gpio_direction_input(BMA2x2_INT);
	data->irq = gpio_to_irq(BMA2x2_INT);
	err = request_irq(data->irq, bma2x2_irq_handler, IRQF_DISABLED|IRQF_TRIGGER_RISING,
			"bma2x2", data);
	if (err)
...
*/
	...
	err = kobject_init_and_add(&data->lge_touch_kobj, &lge_touch_kobj_type,
			data->input->dev.kobj.parent,
			"%s", LGE_TOUCH_NAME);

	enable_irq_wake(data->irq);
	wake_lock_init(&data->wakelock, WAKE_LOCK_SUSPEND, data->input->name);
...
static void bma2x2_early_suspend(struct early_suspend *h)
{
	...
	printk(KERN_ERR "%s : [%d][%d]\n", __func__, atomic_read(&data->tap_enable), atomic_read(&data->enable));

	if (atomic_read(&data->tap_enable) != 1 ) {
	...
	} else if (atomic_read(&data->tap_enable) == 1 ) {
		mutex_lock(&data->enable_mutex);
		bma2x2_set_mode(data->bma2x2_client, BMA2X2_MODE_NORMAL);
		if (atomic_read(&data->enable) == 1 ) {
			cancel_delayed_work_sync(&data->work);
