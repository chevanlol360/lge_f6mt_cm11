...
static void check_log_finger_changed(struct lge_touch_data *ts, u8 total_num)
{
	...
	if (ts->ts_data.prev_total_num != total_num) {
		...
		if (tmp_p != tmp_r
			...
			TOUCH_INFO_MSG("%d finger changed : <%d -> %d> x[%4d] y[%4d]\n",
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->input_dev->name = "lge_synaptics_touch";
 ...
 */
	...
	gpio_tlmm_config(GPIO_CFG(16, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(17, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(52, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
#endif
}
