...
static void as364x_config_gpio_on(void){
	...
	gpio_request(3, "cam_flash_en");
	gpio_tlmm_config(GPIO_CFG(3, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_direction_output(3, 0);
}

static void as364x_config_gpio_off(void){
	pr_err("%s: Start\n", __func__);
...
static void as364x_led_enable(void){
	pr_err("%s: Start\n", __func__);
	gpio_set_value(3, 1);

}

static void as364x_led_disable(void){
	pr_err("%s: Start\n", __func__);
	gpio_set_value(3, 0);
}
