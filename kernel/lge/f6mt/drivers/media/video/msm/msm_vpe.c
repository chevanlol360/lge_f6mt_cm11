	...
	struct v4l2_subdev_fh *fh)
{
	...
	if (frame_info) {
		...
		msm_mctl_unmap_user_frame(&frame_info->dest_frame,
			frame_info->p_mctl->client, mctl->domain_num);
/*LGE_CHANGE_S, mutext crash when terminated recording from GK, 2013-01-29, soojong.jin@lge.com */
...
/*LGE_CHANGE_E, mutext crash when terminated recording from GK, 2013-01-29, soojong.jin@lge.com */
	}
