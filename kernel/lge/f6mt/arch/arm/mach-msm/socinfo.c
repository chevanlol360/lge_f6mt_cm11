...
uint32_t socinfo_get_serial_number(void)
{
    ...
    /* "LG-P650 rev.A" and "LS720 rev.B" are ES1 chipset.
     * This is a unique 32-bit serial number blown into the chip during 
     * wafer sort to track information like wafer number of coordinates of the chip.
     */
    if (SOCINFO_VERSION_MAJOR(socinfo_get_version()) < 0)
    {
        printk("%s: soc version : %d\n", __func__, SOCINFO_VERSION_MAJOR(socinfo_get_version()) );
        return 0;
    }

    tmp = ioremap(SOCINFO_SERIAL_NUM_ADDR, 0x4);
